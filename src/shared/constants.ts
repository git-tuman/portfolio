...
export const PET_PROJECTS_LIST: Project[] = [
  ...
  },
  {
    ...
    stack:
      "TypeScript, React, Redux Toolkit, Ant Design, Vite, Vitest + React Testing Library",
    details: `В роли глобального стейт менеджера выступает Redux Toolkit. Добавление задач из поля ввода по нажатию Enter или по кнопке "+". Количество символов в поле ввода валидируется с помощью логики прописанной во встроенном пропсе rules компонента Form.Item из Ant Design; минимальное количество символов: 5, при попытке ввести меньшее количество - отрисовывается ошибка "Minimum 5 characters!" под полем ввода. Реализована фильтрация задач по спискам: общий, невыполненные задачи, выполненые задачи. Отображение количества оставшихся невыполненых задач. Удаление выполненых задач по клику на кнопку "Clear completed". Основная функциональность (добавление, удаление, фильтрация задач, переключение статуса) протестирована с помощью Vitest и React Testing Library. Это обеспечивает: мгновенную реакцию интерфейса и комфортную работу с задачами, высокую скорость разработки и запуска, уверенность благодаря строгой типизации и облегчённую дальнейшую масштабируемость проекта.`,
  },
];
